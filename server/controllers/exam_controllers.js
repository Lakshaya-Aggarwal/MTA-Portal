...
export const postGroupExam = async (req, res) => {
  try {
    const { title, date, time, venue, type } = req.body;
...
export const viewMyExamDates = async (req, res) => {
  try {
    const { subgroup, group } = req.rootUser;

    const subgroupExams = await Exam.find({ subgroup: subgroup.toString() });
    const groupExams = await Exam.find({ group });

    const allExams = subgroupExams.concat(groupExams);

    res.status(200).json({ exams: allExams });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'An error occurred while fetching exam dates' });
  }
};
